...
class ShopifyService {
  ...
  async verifyProductExistence(sku) {
    try {
      ...
      const exactMatch = products.find(product => product.variants.some(variant => variant.sku === sku));
  ...
  validateImageUrl(url) {
    ...
    try {
      ...
      return parsedUrl.protocol === 'https:' && parsedUrl.hostname === 'images.gammatek.co.za';
    } catch {
      return false;
    }
  }
  ...
  async createOrUpdateProduct(product, stockLevel) {
    try {
      ...
      try {
        ...
        const existingProduct = existingProducts.find(p => p.title.toLowerCase() === modelName.toLowerCase());
        let response;
        if (existingProduct) {
          ...
          const newProductData = {
            ...
            images: Array.isArray(product.images) ? product.images.map(imageUrl => ({
              src: imageUrl,
              attachment: null,
              width: null,
              height: null,
